...
const __ROOT__=path.dirname(path.dirname(new URL(import.meta.url).pathname))+'/';
import * as THREE from '__ROOT__+'node_modules/three/build/three.module.js';

const SRC = '__ROOT__+'src/game.js';
...
const TRACKS = (await import('__ROOT__+'src/tracks.js')).TRACKS;
const name = process.env.TRACK || 'Spa-Francorchamps';
const q = process.env.QUAL || 'HIGH';
const def = { ...TRACKS.find(t => t.name === name) };
const file = `__ROOT__+'public/data/circuits/${def.openf1CircuitKey}.json`;
